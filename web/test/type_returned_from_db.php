...
try {
    ...
    if (!empty($query_error)) {
        $error .= 'The find_by_sql like way failed. The reason given by mysqli is: ' . htmlentities($query_error, ENT_QUOTES | ENT_HTML5);
        die("<p>Died at line 81 with error: {$error}</p>");
    }
} catch (\Exception $e) {
    $error .= ' Caught a thrown exception around line 78: ' . htmlentities($e->getMessage(), ENT_QUOTES | ENT_HTML5) . ' ';
}
